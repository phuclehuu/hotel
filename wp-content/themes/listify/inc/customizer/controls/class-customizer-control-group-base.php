...
<p>
	<select <?php $this->link(); ?> name="<?php echo esc_attr( $name ); ?>">
		...
		<?php foreach ( $this->group as $group_id => $group_data ) : ?>
		<option value="<?php echo $group_id; ?>" <?php echo $this->generate_group_data( $group_data['controls'] ); ?> <?php selected( $group_id, sanitize_title( $this->value() ) ); ?>><?php echo esc_attr( $group_data['title'] ); ?></option>
		<?php endforeach; ?>
